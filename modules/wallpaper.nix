...
let
  ...
  nix-wallpaper-template = {bg, fg1, fg2}: pkgs.writeTextFile {
    name = "template.svg";
    text = ''
...
in {
  ...
  config = (mkIf cfg.generate {
      nixpkgs.overlays = [
        (self: super: {
          generated-nix-wallpaper = self.stdenv.mkDerivation rec {
            ...
            buildInputs = with self; [ inkscape ];
            phases = [ "installPhase" ];
            installPhase = ''
              HOME=/build
              mkdir -p $out
      ...
      wallpaper.path = "${pkgs.generated-nix-wallpaper}/${colorscheme.slug}.png";
    }
  );
}
